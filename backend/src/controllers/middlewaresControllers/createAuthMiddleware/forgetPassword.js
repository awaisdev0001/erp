...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const tanbits_app_email = settings['tanbits_app_email'];
  const tanbits_base_url = settings['tanbits_base_url'];

  const url = checkAndCorrectURL(tanbits_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | tanbits',
    tanbits_app_email,
